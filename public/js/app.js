/* ═══════════════════════════════════════════════════════
   ...
   OPTIMIZED: fast socket, parallel WebRTC, adaptive video,
              ...
              ICE glare fix, ICE candidate queuing, fast rejoin
   ═══════════════════════════════════════════════════════ */
'use strict';
...
const ICE_SERVERS = {
  ...
  iceCandidatePoolSize: 6,   // pre-gather more candidates for faster connect
...
// ICE candidate queue: hold candidates that arrive before remote desc is set
...
async function getLocalMedia() {
  // Stop any existing tracks first so camera is released
  if (localStream) {
...
// ─── ICE Candidate Queue ──────────────────────────────
// FIX: ICE candidates often arrive BEFORE setRemoteDescription is called.
// Without queuing they are silently dropped, causing slow/failed connections.
...
async function drainIceQueue(peerId) {
  const queue = iceQueue[peerId] || [];
  delete iceQueue[peerId];
  const pc = roomPeers[peerId];
  if (!pc) return;
  for (const candidate of queue) {
    try { await pc.addIceCandidate(new RTCIceCandidate(candidate)); }
    catch (e) { console.warn('[Room] Queued ICE add error:', e); }
  }
}

// ─── WebRTC: Room (mesh) ──────────────────────────────
async function createRoomPeerConnection(peerId) {
  // Always clean up any previous connection for this peer
  if (roomPeers[peerId]) {
    roomPeers[peerId].close();
    delete roomPeers[peerId];
  }
  // Clear stale ICE queue for this peer
  delete iceQueue[peerId];
  ...
  pc.ontrack = e => {
    if (e.streams && e.streams[0]) {
      ...
      // Also update big video if this peer is focused
      if (focusedPeerId === peerId) {
        const bigVid = $('room-big-video');
        if (bigVid) { bigVid.srcObject = e.streams[0]; bigVid.muted = false; }
      }
    }
  };
  ...
  // FIX: ICE state indicator + automatic ICE restart on failure
  pc.oniceconnectionstatechange = () => {
    ...
    if (s === 'failed') {
      console.log(`[Room] ICE failed with ${peerId} — restarting ICE`);
      try { pc.restartIce(); } catch (e) { /* not all browsers support */ }
    }
  };
  ...
  pc.onconnectionstatechange = () => {
    ...
    if (s === 'failed') {
      console.log(`[Room] Connection failed with ${peerId} — re-initiating`);
      // Brief delay then re-offer
      setTimeout(() => {
        if (roomPeers[peerId] === pc) {
          initiateRoomOffer(peerId);
        }
      }, 1500);
    }
  };
...
function initSocket() {
  ...
  socket.on('room_joined', ({ roomId, name, members, started }) => {
    ...
    lobbyRoomId.textContent = roomId;
    if (started) {
      // ── REJOIN: session already live — jump straight to call ──
  ...
  socket.on('room_member_joined', ({ socketId, name, members, sessionActive }) => {
    ...
    if (sessionActive && screens.roomChat.classList.contains('active')) {
      ...
      // ── FIX: DO NOT initiate offer here ──
      // The new/rejoining member always initiates (in startRoomSession with isRejoin=true).
      // If we also offer from here, both sides create RTCPeerConnections simultaneously
      // → "ICE glare" → video fails. Let the joiner be the sole initiator.
    }
  });
  ...
  // ── FIX: room_offer handler — close existing stale connection, then respond ──
  socket.on('room_offer', async ({ fromId, offer }) => {
    // Close any existing (possibly stale) connection to this peer
    if (roomPeers[fromId]) {
      roomPeers[fromId].close();
      delete roomPeers[fromId];
    }
  ...
  // ── FIX: ICE candidate queuing ──
  // Candidates often arrive before setRemoteDescription completes.
  // Without queuing they are dropped silently → slow or failed connection.
  socket.on('room_ice', async ({ fromId, candidate }) => {
    ...
    if (!pc.remoteDescription || !pc.remoteDescription.type) {
      ...
      queueIceCandidate(fromId, candidate);
      return;
    }

    try { await pc.addIceCandidate(new RTCIceCandidate(candidate)); }
    catch (e) { console.warn('[Room] ICE add error:', e); }
  });
...
// ─── Start Room Session ───────────────────────────────
// isRejoin = true  → user is reconnecting to a live session
//            false → fresh start from lobby (everyone clicks Start together)
async function startRoomSession(isRejoin = false) {
  roomIdDisplay.textContent = currentRoomId;

  if (isRejoin) {
    // Clean up any dead connections from the previous session
    closeAllRoomPeers();
  }
  ...
  buildRoomVideoLayout();
  reassignStreams();
  addRoomSysMessage(isRejoin ? 'Reconnecting video…' : 'Session started! Video connecting…');

  socket.emit('room_start', { roomId: currentRoomId });

  let targets;
  if (isRejoin) {
    // ── FIX: Rejoin → offer to ALL existing members ──
    // On rejoin we are always the new entrant regardless of position in members list.
    // We initiate to everyone; they respond. No glare possible.
    targets = roomMembers.filter(m => m.socketId !== mySocketId);
  } else {
    // ── Fresh lobby start → index-based pattern to prevent glare ──
    // Each member only offers to members who joined BEFORE them.
    // This guarantees exactly one initiator per pair.
    // Example with [A, B, C]: B offers to A; C offers to A & B. No collisions.
    const myIndex = roomMembers.findIndex(m => m.socketId === mySocketId);
    targets = roomMembers.slice(0, myIndex);
  }

  if (targets.length > 0) {
    // Fire all offers in parallel — no sequential waiting
