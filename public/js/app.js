...
let screenShareStream    = null;   // capture stream (screen + system audio)
let screenShareActive    = false;
let screenShareMixedStream = null; // mixed: screen video + mic + system audio
...
// Close the screen share modal
function closeScreenShareModal() {
  const modal = $('screenshare-modal');
  if (modal) modal.style.display = 'none';
}

// Start screen share — mode: 'screen' | 'window'
async function startScreenShare(preferType) {
  ...
  // getDisplayMedia options — browser will show its native picker
  // preferCurrentTab: false so user can pick screen or window
  const displayOpts = {
    video: {
      cursor: 'always',
  ...
  let captureStream;
  try {
    captureStream = await navigator.mediaDevices.getDisplayMedia(displayOpts);
  } catch (err) {
  ...
  // Mix: screen video + system audio (from capture) + mic audio (from localStream)
  const audioCtx    = new (window.AudioContext || window.webkitAudioContext)();
  const destination = audioCtx.createMediaStreamDestination();

  // Add system/tab audio if captured
  const sysAudioTracks = captureStream.getAudioTracks();
  ...
  if (localStream && localStream.getAudioTracks().length > 0) {
    const micSource = audioCtx.createMediaStreamSource(new MediaStream(localStream.getAudioTracks()));
    micSource.connect(destination);
  }

  // Build the mixed stream: screen video + mixed audio
  ...
  screenShareMixedStream = new MediaStream(tracks);
  ...
  if (peerConnection) {
    ...
    if (sender) await sender.replaceTrack(newVideoTrack).catch(console.warn);
  }

  // Room mode — all room peers
  for (const [peerId, pc] of Object.entries(roomPeers)) {
    const sender = pc.getSenders().find(s => s.track && s.track.kind === 'video');
    if (sender) await sender.replaceTrack(newVideoTrack).catch(console.warn);
    // Also replace audio with mixed
    if (mixedAudioTrack) {
      const aSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
      if (aSender) await aSender.replaceTrack(mixedAudioTrack).catch(console.warn);
    }
  }

  // Replace stranger audio too
  if (peerConnection && mixedAudioTrack) {
    const aSender = peerConnection.getSenders().find(s => s.track && s.track.kind === 'audio');
    if (aSender) await aSender.replaceTrack(mixedAudioTrack).catch(console.warn);
  }

  // Show screen share in local preview
  if (localVideo) localVideo.srcObject = screenShareMixedStream;
  const pipVid = $('duo-pip-vid');
  if (pipVid) pipVid.srcObject = screenShareMixedStream;

  screenShareActive = true;
  updateShareBtnUI(true);
  showToast('🖥 Screen sharing started');

  // When user stops share via browser's native stop button
...
async function stopScreenShare() {
  ...
  if (localStream) {
    ...
    if (origVideoTrack) {
      if (peerConnection) {
        const sender = peerConnection.getSenders().find(s => s.track && s.track.kind === 'video');
        if (sender) await sender.replaceTrack(origVideoTrack).catch(console.warn);
      }
    ...
    if (origAudioTrack) {
      if (peerConnection) {
        const aSender = peerConnection.getSenders().find(s => s.track && s.track.kind === 'audio');
        if (aSender) await aSender.replaceTrack(origAudioTrack).catch(console.warn);
      }
      for (const [, pc] of Object.entries(roomPeers)) {
        const aSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
        if (aSender) await aSender.replaceTrack(origAudioTrack).catch(console.warn);
...
// Update SHARE button appearance on both screens
...
// Trigger buttons — if already sharing, clicking again stops it
...
// Also stop screen share when leaving/ending session
const origLeaveRoom = leaveRoom;
// Patch endSession to also stop screen share
